...
class SimpleDecoder():
	...
	def __call__(self, wav_content: Union[str, np.ndarray, List[str]], **kwargs):
		import ailia
		self.model_fp32 = ailia.Net(weight="sensevoice_small_opset19.onnx", env_id=1)#, enable_layer_fusion = False)

		self.model = ailia.Net(weight="sensevoice_small_fp16_new.onnx", env_id=1)#, enable_layer_fusion = False)
		self.model.set_profile_mode(True)
		...
		# 全てのテンソル名を取得
		tensor_count = self.model.get_blob_count()
		...
		for i in range(tensor_count):
			tensor_name = self.model.get_blob_name(i)
		...
		for tensor_name in tensor_names:
			try:
				data = self.model.get_blob_data(tensor_name)
				data2 = self.model_fp32.get_blob_data(tensor_name)
				#print(f'{tensor_name} , ' + str(np.mean(np.abs(data - data2))))

				#if np.isnan(data).any():
				#	nan_found = True
				#	print(f"[NaN detected] Tensor '{tensor_name}' contains NaN values.")
				#else:
				#	print(f"[OK] Tensor '{tensor_name}'.")
				
				#if tensor_name == "/encoder/after_norm/Add_1_output_0":
				#	print(data)
			except:
				a = 0
