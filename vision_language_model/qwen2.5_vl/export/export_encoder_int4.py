"""
Export Qwen2.5-VL-3B vision encoder int4 quantized ONNX model.
...
This script quantizes the Qwen2.5-VL-3B vision encoder to int4 (4-bit weight-only
quantization).
...
def main():
    parser = argparse.ArgumentParser(
        description="Export Qwen2.5-VL-3B vision encoder int4 quantized model"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory containing original model files (default: same as output_dir)",
    )
    args = parser.parse_args()

    remote_path = "https://storage.googleapis.com/ailia-models/qwen2.5_vl/"
    work_dir = os.path.abspath(args.output_dir)
    input_dir = os.path.abspath(args.input_dir) if args.input_dir else work_dir
    os.makedirs(work_dir, exist_ok=True)

    # Step 1: Download original vision encoder model
    print("[1/3] Downloading original vision encoder model ...")
    original_vis_model = os.path.join(input_dir, "Qwen2.5-VL-3B_vision_encoder.onnx")
    original_vis_pb = os.path.join(input_dir, "Qwen2.5-VL-3B_vision_encoder_weights.pb")
    os.chdir(input_dir)
    download_model("Qwen2.5-VL-3B_vision_encoder.onnx", remote_path)
    download_model("Qwen2.5-VL-3B_vision_encoder_weights.pb", remote_path)

    # Step 2: Convert Gemm to MatMul+Add and quantize to int4
    print("[2/3] Quantizing vision encoder to int4 ...")
    quantized_vis_model = os.path.join(work_dir, "Qwen2.5-VL-3B_vision_encoder_int4.onnx")
    ...
    if os.path.exists(quantized_vis_model):
        os.remove(quantized_vis_model)
