"""
Export Qwen2.5-VL-3B LLM decoder int4 quantized ONNX model.
...
This script quantizes the Qwen2.5-VL-3B LLM decoder to int4 (4-bit weight-only
...
def main():
    parser = argparse.ArgumentParser(
        description="Export Qwen2.5-VL-3B decoder int4 quantized model"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    original_model = "Qwen2.5-VL-3B_language_model.onnx"
    original_pb = "Qwen2.5-VL-3B_language_model_weights.pb"
    ...
    quantized_model = os.path.join(work_dir, "Qwen2.5-VL-3B_language_model_int4.onnx")
    ...
    weights_name = os.path.basename(quantized_model).replace(".onnx", "_weights.pb")
    weights_path = os.path.join(work_dir, weights_name)
    for path in (quantized_model, weights_path):
